...
def filtered_path_generator(X0, A, C, K, Z, N):
    #  X0 -- initial state, A -- state transition matrix, C -- observation matrix, \
    #  K -- sequence of Kalman gains, Z -- sequence of observations, N -- number of total time steps
    ...
    for n in range(N):
        ...
        Kn = np.array(K[:, n+1], ndmin=2)  # add one dimension
        X_hat[:, n + 1] = A @ X_hat[:, n] + (Z[:, n + 1] - Z_hat) @ Kn  # state estimate
