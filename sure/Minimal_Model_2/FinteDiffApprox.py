...
import numpy as np
"""this page will be replaced by FDA.py"""


def FDA_control_2(init_cond, param_list, control_gain, Filter, total_noise, Gb, Ib, N_meas, T, T_list, N,
                meal_params, lbda, r):
    ...
    for i in range(len(control_gain)):
        ...
        state_variable_f, Z_f, true_state_f = generate_path_whole(init_cond, param_list, control_shift_f, Filter,
                                                                  total_noise, Gb,
        ...
        state_variable_b, Z_b, true_state_b = generate_path_whole(init_cond, param_list, control_shift_b, Filter,
                                                                  total_noise,
                                                                  Gb, Ib, N_meas, T, T_list, N, meal_params)
        cost_b = cost_computation(true_state_b, state_variable_b, Gb, Ib, control_gain, lbda, r, ub=140, lb=80)  # J(a-s)
...
def FDA_filter_2(init_cond, param_list, control_gain, Filter, total_noise, Gb, Ib, N_meas, T, T_list, N,
                meal_params, lbda, r):
    ...
    for i in range(len(Filter)):
        ...
        state_variable_f, Z_f, true_state_f = generate_path_whole(init_cond, param_list, control_gain, filter_shift_f,
                                                                  total_noise,
                                                                  Gb, Ib, N_meas, T, T_list, N, meal_params)
        cost_f = cost_computation(true_state_f, state_variable_f, Gb, Ib, control_gain, lbda, r, ub=140, lb=80)  # J(a+s)
        state_variable_b, Z_b, true_state_b = generate_path_whole(init_cond, param_list, control_gain, filter_shift_b,
                                                                  total_noise,
                                                                  Gb, Ib, N_meas, T, T_list, N, meal_params)
        cost_b = cost_computation(true_state_b, state_variable_b, Gb, Ib, control_gain, lbda, r, ub=140, lb=80)  # J(a-s)
        dJ_da = (cost_f - cost_b) / (a-b)   # [J(a)-J(b)]/ 2(a-b)
...
# def FDA_param_2(init_cond, param_list, control_gain, Filter, total_noise, Gb, Ib, N_meas, T, T_list, N, meal_params):
