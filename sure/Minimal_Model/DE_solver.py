...
def GXI_system2(x, t, p1, p2, p3, Gb, Ib):
    """ When I(t) is determined by linear approximation of insulin data
     glucose (G) - remote insulin (X) - plasma insulin (I) system
     :param x          - x = [G, X, I]
     :param p1, p2, p3 - bio parameters (unit: min^-1, min^-1, min^-1 mU/l)
     :param Gb, Ib     - basal plasma glucose (mmol/l) and insulin (mU/l)
...
def G_X_sys(u, Input_Index, Plot):
    ...
    if Input_Index==0:
        sol_I  = phar_kin.solveODE(u, Plot=False)  # Get I(t) first, then use I(t) to solve the ODE sys of G and X
    ...
    if Plot:
        ...
        plt.xlabel("t [min]")
        plt.ylabel("G [mmol/l]")   #1 mg/dL = 0.0555 mmol/L
        plt.grid(True)
        ...
        #Plot Ra(t) Glucose input
        plt.plot(t, Ra(t))
        plt.xlabel("t [min]")
        plt.ylabel("Ra [mmol/min]")
        plt.grid(True)
        plt.title("Glucose Input Ra(t)")
        plt.show()
    return G, X
