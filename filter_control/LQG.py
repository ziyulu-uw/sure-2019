...
from LQG_tool import generate_noise
...
def LQG_simulation(x0,v0,A,B,C,R,S,r,Sn_list,n,N):
    ...
    @return:    X_val n*N*2 all the x,v
                K_val n*N*2 all the Kalman Gain
                G_val n*N*2 all the Control Gain'''
    ...
    G_val     = np.zeros([N,2])      # a tensor to take all G val at all time
    X_val     = np.zeros([n,N,2])    # a tensor to take all Xn = [xn;vn]
    TrueX_val = np.zeros([n,N,2])    # true value of X
    ...
    # W = generate_noise(R,n,N) #process noise: R-cov  [Wn is not used in the simulation]
    V = generate_noise(S,n,N) #observation nois: S-cov
    ...
    for k in range(N):
        ...
        # ----debuging: Tn shoud be equal to ATA+R-------------
        #update covariance matrix of Yn
        #Tn          = (A-Kn@C@A)@Tn@transpose(A-Kn@C@A)+(I-Kn@C)@R@transpose(I-Kn@C)+Kn@S@transpose(Kn) 
        #print("ATA+R:",(A@Tn@A.T+R)[0,:,:])
        #print("Pn:", Pn[0,:,:])
        #Kn_         = (A@Tn@A_T+R)@C_T@la.inv(C@A@Tn@A_T@C_T+C@R@C_T+S)
